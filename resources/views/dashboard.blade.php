    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
            <a href="{{ route('checkin.form') }}"
               class="bg-white rounded-xl border border-gray-150 p-6 text-center hover:shadow-md transition duration-200 group relative overflow-hidden">
            ...
            <a href="{{ route('checkout.form') }}"
               class="bg-white rounded-xl border border-gray-150 p-6 text-center hover:shadow-md transition duration-200 group relative overflow-hidden">
                ...
                <p class="text-xs text-gray-400 mt-1">Log ingredient extraction & trigger FIFO reduction</p>
            </a>
            ...
            <a href="{{ route('products.index') }}"
               class="bg-white rounded-xl border border-gray-150 p-6 text-center hover:shadow-md transition duration-200 group relative overflow-hidden">
                ...
                <p class="text-xs text-gray-400 mt-1">Audit active store ledgers & global balances</p>
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="text-gray-400 text-[10px] font-black uppercase tracking-wider border-b border-gray-150">
                                <th class="pb-3">Item Descriptor</th>
                                <th class="pb-3">Action Type</th>
                                <th class="pb-3">Adjustment</th>
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="py-3">
                                @if($t->type === 'check_in')
                                    <span class="inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-100">Check-In</span>
                                @elseif($t->type === 'check_out')
                                    <span class="inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-100">Check-Out</span>
                                @else
                                    <span class="inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-rose-50 text-rose-700 border border-rose-100">{{ $t->type }}</span>
                                @endif
                            </td>
                            <td class="py-3 font-black text-sm {{ $t->type === 'check_in' ? 'text-emerald-600' : 'text-rose-600' }}">
                        ...
                        <tr>
                            <td colspan="4" class="py-10 text-center text-gray-400 text-xs italic">
                                No activity recorded during this transaction cycle.
                            </td>
                        </tr>
                        @endforelse
                        </tbody>
            ...
            {{-- System Low Stock Alerts Column --}}
            <div class="lg:col-span-2 bg-white rounded-xl border border-gray-150 shadow-sm p-6">
                <div class="mb-4">
                    <h3 class="font-black text-gray-900 text-sm uppercase tracking-wider">Automated Low Stock Alerts</h3>
                ...
                @forelse($lowStockProducts as $p)
                    <div class="bg-rose-50/60 border border-rose-100 rounded-xl px-4 py-3 flex items-center justify-between shadow-inner">
                        ...
                        <span class="text-xs font-black bg-rose-100 text-rose-700 px-2 py-1 rounded-md">
                            {{ $p->stock }} {{ $p->unit }} left
                        </span>
                    </div>
                @empty
                    <div class="text-center py-12">
                        ...
                        <p class="text-[11px] text-gray-400 mt-0.5">All master ingredients are perfectly provisioned.</p>
                    </div>
                @endforelse
                </div>
            </div>
        ...
        {{-- ─── Expiry Reminder Component Table ─── --}}
        <div class="bg-white rounded-xl border border-gray-150 shadow-sm p-6 overflow-hidden">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-5">
                <div>
                    <h3 class="font-black text-gray-900 text-sm uppercase tracking-wider">Dynamic Shelf Expiry Monitor</h3>
                ...
                <table class="w-full text-sm text-left">
                    <thead>
                        <tr class="text-gray-400 text-[10px] font-black uppercase tracking-wider border-b border-gray-150">
                            ...
                            <th class="pb-3 text-right">Horizon Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                    @forelse($expiringItems as $item)
                    <tr class="hover:bg-gray-50/50 transition">
                        {{-- Handles data variation elegantly if the record is a Product model or a direct StockBatch --}}
                        <td class="py-3.5 font-bold text-gray-900">
                            {{ method_exists($item, 'batches') ? $item->name : ($item->product->name ?? 'Unknown Item') }}
                        </td>
                        <td class="py-3.5">
                            <span class="text-xs font-medium bg-gray-100 text-gray-600 px-2 py-0.5 rounded">
                                {{ method_exists($item, 'batches') ? $item->category : ($item->product->category ?? '—') }}
                            </span>
                        </td>
                        <td class="py-3.5 font-semibold text-gray-700">
                            {{ method_exists($item, 'batches') ? $item->stock : ($item->remaining_quantity ?? $item->quantity ?? '0') }}
                            <span class="text-xs text-gray-400 font-normal uppercase ml-0.5">
                                {{ method_exists($item, 'batches') ? $item->unit : ($item->product->unit ?? '') }}
                            </span>
                        </td>
                        <td class="py-3.5 font-mono text-xs text-gray-600">
                        ...
                        <td class="py-3.5 text-right">
                            ...
                            @endphp
                            @if($daysLeft <= 0)
                                <span class="inline-block text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded bg-rose-100 text-rose-700 border border-rose-200 animate-pulse">Critical (Expired)</span>
                            @elseif($daysLeft === 1)
                                <span class="inline-block text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded bg-rose-50 text-rose-600 border border-rose-100">Tomorrow</span>
                            @elseif($daysLeft <= 3)
                                <span class="inline-block text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded bg-amber-100 text-amber-700 border border-amber-200">{{ $daysLeft }} Days Left</span>
                            @else
                                <span class="inline-block text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded bg-yellow-50 text-yellow-700 border border-yellow-200">{{ $daysLeft }} Days Left</span>
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="py-12 text-center text-gray-400 text-xs">
                            ✨ Safe Zone: No batch records are flagged for expiration inside the current 7-day tracking window.
                        </td>
                    </tr>
                    @endforelse
                    </tbody>
